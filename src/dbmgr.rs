////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// dbmgr -- create, initialize and add sample data to a database
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
use crate::{
    dbconn::DBConfig,
    schema::{SchemaDef, SchemaDefFactory, TableDef},
};
use log::{debug, error, info};

use mysql::{Params, Pool, PooledConn, prelude::Queryable};

pub trait SampleDataLoader {
...
pub fn do_operation<T: SchemaDefFactory, L: SampleDataLoader>(
    cfg: &AppConfig,
    fact: &T,
    loader: &L,
) {
    match cfg.op {
        Op::Create(i, c, s) => {
            let schema = fact.build_schema_def();
            initialize(&cfg, &schema, loader, (i, c, s));
        }
        Op::Drop => {
            drop_db(&cfg);
        }
...
fn drop_db(cfg: &AppConfig) {
    ...
    let drop_db_stmt = stmt_drop_db(&cfg);
    debug!(target: "drop_db", "{:?}", drop_db_stmt);
    match root_conn.exec_drop(drop_db_stmt, Params::Empty) {
        Ok(_) => None,
...
fn initialize<T: SampleDataLoader>(
    cfg: &AppConfig,
    schema: &SchemaDef,
    loader: &T,
    (init_db, init_schema, load_sample): (bool, bool, bool),
) -> bool {
    let mut all_pass = true;

    if all_pass && init_db {
        all_pass = initialize_db(cfg);
    }

    if all_pass && init_schema {
        all_pass = initialize_schema(cfg, &schema);
    }

    if all_pass && load_sample {
        match initialize_data(cfg, loader) {
            Some(err) => {
                eprintln!("Error loading sample data: {err}");
                all_pass = false;
            }
            None => {}
        }
...
fn initialize_data<T: SampleDataLoader>(cfg: &AppConfig, loader: &T) -> Option<String> {
    ...
    loader.load_sample_data(&mut conn)
}

fn initialize_schema(cfg: &AppConfig, def: &SchemaDef) -> bool {
    let user_url = cfg.as_user_url();
...
fn initialize_db(cfg: &AppConfig) -> bool {
    let root_url = cfg.as_root_url();
...
pub fn process_parameters(cfg: &mut AppConfig) -> bool {
